                ...
                <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center max-w-7xl mx-auto">
                    ...
                    <div className={`transform transition-all duration-1000 ${hasIntersected ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                        ...
                        <div className="inline-flex items-center space-x-2 px-4 py-2 rounded-full bg-blue-50 border border-blue-200 mb-8">
                            ...
                            <span className="text-xs font-medium text-blue-700">Disponible pour de nouveaux projets</span>
                        </div>

                        {/* Main Heading */}
                        <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold leading-tight mb-6">
                            <span className="text-gray-900 block">ENGINEERING</span>
                            <span className="text-blue-600 block">MEANINGFUL</span>
                            <span className="text-gray-900 block">EXPERIENCES</span>
                        </h1>

                        {/* Tagline */}
                        <p className="text-xl md:text-2xl text-gray-600 mb-8 font-light">
                            Full-Stack Developer • {displayedText}
                            <span className={`inline-block w-0.5 h-6 bg-blue-600 ml-1 ${isTyping ? 'opacity-100' : 'animate-pulse'}`} />
                        </p>
                        ...
                        <div className="flex flex-wrap gap-4">
                            <Link
                                to="/projects"
                                className="px-8 py-4 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
                            >
                                Voir mes projets
                            </Link>
                            <Link
                    ...
                    <div className={`relative transform transition-all duration-1000 delay-200 ${hasIntersected ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                        <div className="relative group">
                            ...
                            <div className="absolute -inset-4 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-2xl opacity-10 group-hover:opacity-20 transition-opacity duration-300" />
                            ...
                            <div className="relative aspect-[4/5] w-full max-w-md mx-auto overflow-hidden rounded-2xl shadow-2xl">
                                <img
                                    ...
                                    onError={(e) => {
                                        e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(PERSONAL_INFO.name)}&size=800&background=3B82F6&color=ffffff`;
                                    }}
                                />
                            </div>

                            {/* Info Badge */}
                            <div className="absolute bottom-6 left-6 right-6 p-4 bg-white/95 backdrop-blur-sm rounded-xl shadow-lg border border-gray-100">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="font-bold text-gray-900">{PERSONAL_INFO.name}</p>
