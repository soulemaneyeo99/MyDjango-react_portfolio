        ...
        <section ref={targetRef} className="py-24 bg-[#050505] relative overflow-hidden">
            ...
            <div className="container-custom relative z-10">
                <div className={`mb-20 grid md:grid-cols-[1fr_auto] gap-8 items-end transform transition-all duration-1000 ${hasIntersected ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                    <div>
                        <div className="font-mono text-[10px] text-indigo-400 uppercase tracking-[0.4em] mb-4">Selected Works</div>
                        <h2 className="text-4xl md:text-7xl font-bold tracking-tighter">
                            FEATURED <span className="text-indigo-500 italic">PROJECTS</span>
                        </h2>
                    </div>
                    <Link to="/projects" className="group flex items-center space-x-4 text-xs font-mono tracking-widest text-slate-500 hover:text-white transition-colors pb-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-1px bg-white/5 border border-white/5">
                    {featuredProjects.map((project, index) => (
                        <div
                            key={project.id}
                            className={`group relative aspect-video overflow-hidden bg-slate-900 transform transition-all duration-1000 ${hasIntersected ? 'opacity-100' : 'opacity-0'}`}
                        ...
                        >
                            ...
                            <div className="absolute inset-0 p-8 flex flex-col justify-between bg-gradient-to-t from-black via-black/20 to-transparent">
                                <div className="flex justify-between items-start">
                                    <span className="px-3 py-1 bg-white/5 backdrop-blur-md border border-white/10 text-[10px] font-mono text-indigo-400">
                                    ...
                                    <div className="flex space-x-4 opacity-0 group-hover:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0 duration-300">
                                        ...
                                        <Link to={`/projects/${project.id}`} className="p-2 bg-indigo-500 hover:bg-indigo-600 transition-colors">
                                ...
                                <div className="space-y-4">
                                    <h3 className="text-2xl md:text-3xl font-bold text-white tracking-tight group-hover:text-indigo-400 transition-colors">
                                        {project.title}
                                    </h3>
                                    <p className="text-slate-400 text-sm max-w-md line-clamp-2 font-light opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0">
                                        {project.description}
                                    </p>
                                    <div className="flex flex-wrap gap-2 pt-2">
                                        {project.techStack.slice(0, 3).map(tech => (
                                            <span key={tech} className="text-[9px] font-mono text-slate-500 uppercase tracking-widest border border-white/10 px-2 py-1 bg-black/50">
                                                {tech}
                                            </span>
                                        ))}
                                    </div>
                            ...
                            <div className="absolute bottom-0 left-0 w-0 h-[2px] bg-indigo-500 group-hover:w-full transition-all duration-700" />
                        </div>
                    ))}
                </div>
